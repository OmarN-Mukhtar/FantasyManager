    ...
    try:
        ...
        if rag.load_data():
            ...
            if not rag._load_index():
                return None  # Silently return None if no index
            return rag
        return None
    except Exception as e:
        return None  # Silently return None on any error
...
def main():
    ...
    with tab4:
        ...
        if rag is None:
            st.info("📊 **RAG Search Currently Unavailable**")
            st.markdown("""
            The vector database hasn't been created yet. This feature requires:
            
            1. **Player data** (fetched from FPL API)
            2. **News articles** (collected from Google News)
            3. **Predictions** (generated by ML models)
            4. **Vector database** (FAISS index)
            
            **To enable RAG search:**
            - The daily automated pipeline will create this database soon
            - Or run locally: `python src/rag_system.py`
            
            In the meantime, you can explore the **Player Table**, **Predictions**, and **Analytics** tabs!
            """)
        else:
